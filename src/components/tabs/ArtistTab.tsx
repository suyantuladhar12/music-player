...
export function ArtistTab({
  ...
  const [drilledArtist, setDrilledArtist] = useState<string | null>(null);
  ...
  const openDrilldown = (artist: string) => {
    setDrilledArtist(artist);
  ...
  const handleSelectArtist = (artist: string) => {
    onSelectSong(null);
    onSelectArtist(artist === selectedArtist ? null : artist);
  };
  ...
  const visibleArtists = artists
    .filter((artist) => !query || artist.toLowerCase().includes(query))
    .sort(
      (a, b) =>
              ...
              return (
                ...
                >
                  <div style={styles.songInfo}>
                    ...
                    <div style={styles.songSub}> {song.album ?? ""} </div>
                  </div>
  ...
  return (
    <div style={styles.list}>
      ...
      {!isLoading && !error && visibleArtists.length === 0 && (
        <div style={styles.status}>
          {query ? "No matching artists" : "No artists found"}
        </div>
      )}
      {!isLoading &&
        !error &&
